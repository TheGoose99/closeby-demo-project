...
export function BookingSection({ config }: { config: ClientConfig }) {
  ...
  const { calComUsername, calComCanonicalEventSlugs, calComEventSlugs, whatsappNumber, whatsappMessage } = effectiveConfig.integrations
  ...
  useEffect(() => {
    setCalReady(false)
    // Keep OTP verification state for the current page instance.
    // Users should not be asked to verify again when switching service options.
    setGuardError(null)
  }, [calLink])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      cal('ui', {
        theme: 'light',
        cssVarsPerTheme: { light: { 'cal-brand': '#4d7a5e' }, dark: { 'cal-brand': '#7a9e87' } },
  ...
  async function handleSendSms(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await startPhoneOtp(auth, phoneForGuard.trim(), FIREBASE_PHONE_RECAPTCHA_BUTTON_ID)
      recaptchaVerifierRef.current = result.verifier
  ...
  async function handleVerifyOtp(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const verifyResponse = await fetch('/api/anti-abuse/verify-phone', {
        method: 'POST',
    ...
    <section id="programare" className="py-24 px-6 lg:px-10 bg-ink">
      <div className="max-w-[1200px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            <span className="text-xs font-medium tracking-[0.1em] uppercase text-white/40 block mb-4">Programare</span>
            <h2 className="font-serif text-4xl md:text-5xl font-medium text-white mb-4 leading-tight">
              Primul pas este <em className="text-sage italic">cel mai important</em>
            </h2>
            <p className="text-white/55 text-base mb-10">Prima consultație este gratuită. Alege tipul ședinței și rezervă direct în calendar.</p>
              ...
              {bookingOptions.map((opt) => (
                <button
                  ...
                  className={cn(
                    ...
                    selected === opt.key ? 'bg-sage-d/20 border-sage-d text-white' : 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10 hover:border-white/20',
                  )}
                >
                  <div className={cn('w-11 h-11 rounded-lg flex items-center justify-center text-xl flex-shrink-0', selected === opt.key ? 'bg-sage-d' : 'bg-white/10')}>
                    {opt.icon}
                  </div>
                  <div>
                    <div className="font-medium text-sm">{opt.label}</div>
                    <div className={cn('text-xs mt-0.5', selected === opt.key ? 'text-white/60' : 'text-white/35')}>{opt.note}</div>
                  </div>
                  {selected === opt.key && <span className="ml-auto text-sage text-lg">✓</span>}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              <a href={`tel:${effectiveConfig.phone}`} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 hover:bg-white/10 transition-colors">
              ...
              {whatsappNumber && (
                <a href={waUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 hover:bg-white/10 transition-colors">
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-[0_24px_64px_rgba(0,0,0,0.35)]">
            <div className="px-6 pt-6 pb-4 border-b border-sage-l/30">
              <h3 className="font-serif text-xl font-medium text-ink">{selectedOption?.label}</h3>
              <p className="text-sm text-ink-l mt-1">{selectedOption?.note} · {effectiveConfig.address.sector} sau online</p>
              <p className="text-xs text-ink-xl mt-2">Durata exactă este setată în Cal.com.</p>
            </div>
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-sage-xl">
                  {phoneStep === 'phone' ? (
                    ...
                    <form onSubmit={handleVerifyOtp} className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
                      ...
                      <p className="mt-2 text-sm text-ink-l">Ti-am trimis un cod la {phoneForGuard}</p>
                      <label className="mt-4 block text-xs font-medium uppercase tracking-wide text-ink-l">Cod</label>
                      <input inputMode="numeric" autoComplete="one-time-code" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} className="mt-1 w-full rounded-lg border border-sage-l px-3 py-2 text-sm text-ink outline-none focus:border-sage-d" placeholder="123456" />
                      {guardError && <p className="mt-2 text-xs text-red-700">{guardError}</p>}
                      <div className="mt-4 flex gap-2">
                        ...
                        <button type="submit" disabled={isCheckingGuard} className="inline-flex flex-1 items-center justify-center rounded-lg bg-sage-d px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
                          {isCheckingGuard ? 'Verificam...' : 'Verifica si continua'}
                        </button>
                      </div>
                    </form>
                  )}
            ...
            <div className="px-6 py-4 bg-sage-l/40 border-t border-sage-l/30 flex items-center gap-2">
              ...
              <span className="text-xs text-sage-d/80">
                Date stocate pe servere in {effectiveConfig.gdpr.serverLocation} · GDPR compliant · {effectiveConfig.gdpr.dataProcessorName}
              </span>
            </div>
          </div>
        </div>
