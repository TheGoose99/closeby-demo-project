...
/**
 * @param {{ services: Array<{ title: string; duration: number; price?: number; currency: string; calEventSlug: string; icon: string }>; integrations: { calComEventSlugs: Record<EventSlugKey, string> } }} config
 * @param {EventSlugKey} key
 */
function findServiceByKey(config, key) {
  const slug = config.integrations.calComEventSlugs[key]
